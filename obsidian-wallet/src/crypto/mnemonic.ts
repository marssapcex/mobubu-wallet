...
export function seedToMnemonic(seed: Uint8Array): string {
  ...
  // Calculate 25th checksum word
  // Monero uses the first 3 characters of each of the 24 words
  let prefixString = '';
  for (let i = 0; i < 24; i++) {
    const word = words[i];
    prefixString += word.slice(0, 3);
  }
...
export function mnemonicToSeed(mnemonic: string): Uint8Array {
  ...
  // If 25 words provided, verify checksum
  ...
  for (let i = 0; i < 24; i += 3) {
    ...
    if (w1 === undefined || w2 === undefined || w3 === undefined) {
      throw new Error(`Unrecognized word in mnemonic at position ${i + 1}-${i + 3}`);
    }
