...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete }) => {
  ...
  const [step, setStep] = useState<'welcome' | 'password' | 'seed' | 'node' | 'complete'>('welcome');
  ...
  const handleFinalize = async (seedToUse?: string) => {
    try {
      await createWallet(password, seedToUse);
      setStep('complete');
    } catch (err) {
    ...
    <div className="min-h-full flex flex-col justify-center p-4 bg-slate-950 text-slate-100 select-none">
      <div className="w-full max-w-sm mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5">
        ...
        <div className="flex flex-col items-center text-center space-y-2">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-orange-600 via-amber-500 to-yellow-400 p-[2px] shadow-lg shadow-orange-500/20">
            <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center font-black text-amber-500 text-xl">
              ɱ
            </div>
          </div>
          ...
          <p className="text-xs text-slate-400">
            The MetaMask for Monero with Feather Tor backbone
          </p>
        </div>
          ...
          <div className="space-y-3 pt-2">
            <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1.5 text-xs">
              <div className="flex items-center gap-1.5 text-orange-400 font-bold">
                ...
                <span>Next-Gen Monero Browser Extension</span>
              </div>
        ...
        {step === 'password' && (
          <form onSubmit={handlePasswordSubmit} className="space-y-3.5">
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              This password encrypts your keys locally with AES-256-GCM. It will be required each time you unlock Obsidian.
            </p>
        ...
        {step === 'node' && (
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Obsidian defaults to Tor Onion routing to ensure your IP address is never revealed to node operators.
            </p>
        ...
        {step === 'complete' && (
          <div className="text-center space-y-4 py-2">
            ...
            <div>
              <h2 className="text-base font-bold text-white">Wallet Initialized!</h2>
