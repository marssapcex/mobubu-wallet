...
export const SwapModal: React.FC<SwapModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleStartSwap = async () => {
    ...
    const order = await createSwap(fromCoin, toCoin, selectedQuote, currentAddress);
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto shadow-2xl flex flex-col text-slate-100">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded px-1.5 py-0.2 font-mono">
                  Cake-Style Aggregator
                </span>
              </div>
              <p className="text-[11px] text-slate-400">Zero-KYC Instant Exchange</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {activeOrder ? (
              ...
              <div className="text-center text-xs">
                ...
                {activeOrder.status === 'confirming' && (
                  <span className="text-blue-300 animate-pulse">
                    ⛓️ Detected on blockchain! Awaiting 1 confirmation...
                  </span>
                )}
                {activeOrder.status === 'exchanging' && (
                  <span className="text-purple-300 animate-pulse">
                    🔄 Exchanging to Monero & generating stealth outputs...
                  </span>
                )}
          ...
          <div className="p-4 space-y-4">
            ...
            {/* Cake Wallet Provider Aggregator Comparison */}
            <div className="space-y-2">
            ...
            {/* Recipient Subaddress Preview */}
            <div className="p-2.5 bg-slate-950/70 border border-slate-800 rounded-xl text-xs space-y-1">
              <div className="text-slate-400 text-[11px]">Payout Stealth Address:</div>
