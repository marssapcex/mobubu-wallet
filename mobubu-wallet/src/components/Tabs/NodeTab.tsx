...
export const NodeTab: React.FC = () => {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-4 shadow-sm">
        ...
        <div className="space-y-1.5 pt-2 border-t border-slate-700/60 text-xs">
          ...
          <div className="flex items-center justify-between text-[10px] text-slate-500">
            ...
            <span>Fluorine Fermi v0.18</span>
          </div>
        </div>
      </div>
      ...
      {/* Node Presets Selection with Benchmark */}
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider">
          <span>Available Nodes ({nodes.length})</span>
          <button
            onClick={handleBenchmark}
            disabled={isBenchmarking}
            className="flex items-center gap-1 text-[11px] text-orange-400 hover:text-orange-300 font-semibold normal-case cursor-pointer"
          >
            <Zap className={`w-3 h-3 ${isBenchmarking ? 'animate-spin' : ''}`} />
