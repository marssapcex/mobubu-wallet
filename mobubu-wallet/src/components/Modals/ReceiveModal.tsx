...
import { createIntegratedAddress } from '../../crypto/keys';
...
export const ReceiveModal: React.FC<ReceiveModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleToggleIntegrated = () => {
    if (!showIntegrated) {
      try {
        const addr = createIntegratedAddress(activeAccount.primaryAddress);
        setIntegratedAddress(addr);
        ...
        <div className="p-5 flex flex-col items-center space-y-4">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${
              ...
              {showIntegrated 
                ? 'Integrated Address (PID Embedded)' 
                : isSubaddress 
                ? `Subaddress #${currentSub?.minor || 1}` 
                : '⚠️ Main Address'}
            </span>
            <span className="text-xs text-slate-400 font-medium">
              {!showIntegrated ? (currentSub?.label || 'Default') : 'Merchant Payment'}
            </span>
          </div>
          ...
          {/* Actions: Fresh Subaddress & Integrated Address */}
          <div className="w-full space-y-2">
            <button
              onClick={handleCreateSubaddress}
              className="w-full bg-purple-900/60 hover:bg-purple-800/80 border border-purple-700/60 text-purple-200 text-xs font-semibold py-2 rounded-xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              onClick={handleToggleIntegrated}
              className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium py-1.5 rounded-xl border border-slate-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <Tag className="w-3.5 h-3.5 text-blue-400" />
              <span>{showIntegrated ? 'Switch back to Stealth Subaddress' : 'Generate Integrated Address (106 chars)'}</span>
            </button>
          </div>
