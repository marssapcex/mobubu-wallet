...
export const NodeSelectorModal: React.FC<NodeSelectorModalProps> = ({ isOpen, onClose }) => {
  const { nodes, activeNode, switchNode, addCustomNode, benchmarkAllNodes } = useWallet();
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center justify-between p-2.5 bg-slate-950/70 border border-slate-800 rounded-xl">
            <div className="text-xs text-slate-300 flex items-center gap-1.5">
              ...
              <span>Test Latencies & Health</span>
            </div>
            <button
              onClick={handleBenchmark}
              disabled={isBenchmarking}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs font-semibold text-slate-200 flex items-center gap-1 cursor-pointer disabled:opacity-50"
            >
              <RotateCw className={`w-3 h-3 ${isBenchmarking ? 'animate-spin' : ''}`} />
              <span>{isBenchmarking ? 'Testing...' : 'Ping All'}</span>
            </button>
          </div>
              ...
              {nodes.filter(n => n.type === 'local_fullnode').map(node => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-2.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${
                      isSelected
                        ? 'bg-blue-950/40 border-blue-500/80 shadow-sm'
                        : 'bg-slate-950/60 hover:bg-slate-800/60 border-slate-800 text-slate-400'
                    }`}
                  >
                    <div>
