...
import { validateMoneroAddress, resolveOpenAlias } from '../../crypto/keys';
...
export const SendModal: React.FC<SendModalProps> = ({ isOpen, onClose }) => {
  ...
  const [openAliasName, setOpenAliasName] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (trimmed.includes('@') || (trimmed.includes('.') && !trimmed.startsWith('4') && !trimmed.startsWith('8'))) {
      resolveOpenAlias(trimmed).then(res => {
        if (res) {
          setResolvedAddress(res.address);
          setOpenAliasName(res.recipientName || 'OpenAlias Verified');
        } else {
          setResolvedAddress(trimmed);
          setOpenAliasName(null);
        }
      });
    } else {
      setResolvedAddress(trimmed);
  ...
  const addrToValidate = resolvedAddress || recipientInput.trim();
  const addrValidation = addrToValidate ? validateMoneroAddress(addrToValidate) : null;
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (!addrToValidate) {
      setErrorMsg('Please enter a recipient Monero address or OpenAlias');
      return;
    }
    ...
    if (totalDeduction > activeAccount.unlockedBalanceXMR) {
      setErrorMsg(`Insufficient unlocked balance. Need ${totalDeduction.toFixed(6)} XMR`);
      return;
    }

    setIsSending(true);
    try {
      const tx = await sendTransaction(addrToValidate, numAmount, priority);
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col text-slate-100">
        ...
        <div className="p-4 border-b border-slate-800 flex items-center justify-between sticky top-0 bg-slate-900/95 backdrop-blur-md z-10">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-white">Send Monero (XMR)</h2>
              <p className="text-[11px] text-slate-400">Private RingCT Transaction (16 Decoys)</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="p-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                Sent {numAmount} XMR with 16 ring decoys routed through{' '}
                {activeNode.isTor || isTorRoutingEnabled ? 'Tor Hidden Service' : 'Clearnet'}.
              </p>
            </div>
          ...
          <form onSubmit={handleSend} className="p-4 space-y-3.5">
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                <span>Recipient Address or OpenAlias</span>
                {openAliasName && (
                  <span className="text-[10px] text-emerald-400 font-semibold flex items-center gap-1">
                    <AtSign className="w-3 h-3" />
                    <span>{openAliasName}</span>
                  </span>
                )}
                {addrValidation && addrValidation.isValid && !openAliasName && (
                  <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded ${
                    addrValidation.type === 'subaddress' 
                      ? 'bg-purple-950/80 text-purple-300 border border-purple-800' 
                      : addrValidation.type === 'integrated'
                      ? 'bg-blue-950/80 text-blue-300 border border-blue-800'
                      : 'bg-amber-950/80 text-amber-300 border border-amber-800'
                  }`}>
                    {addrValidation.type === 'subaddress' ? 'Stealth Subaddress (8...)' : addrValidation.type === 'integrated' ? 'Integrated (PID embedded)' : 'Primary Address (4...)'}
                  </span>
                )}
              </label>
              <textarea
                rows={2}
                placeholder="4... or 8... or donate@getmonero.org"
